...
class ChatReadServiceImpl(
    ...
    override fun markUnread(
        ...
        val markerMessage = if (fromMessageId != null) {
            val message = chatMessageDao.findById(fromMessageId)
                .orElseThrow {
                    InteractionNotFoundException(
                        message = "Сообщение чата не найдено",
                        code = "chat_message_not_found",
            ...
            chatMessageDao.findTopByDialogIdAndSenderUserIdNotOrderByIdDesc(
                dialogId = dialogId,
                senderUserId = currentUser.userId,
            ) ?: return
        }

        val markerId = markerMessage.id
            ?: throw InteractionInternalException(
                message = "Не найден идентификатор сообщения чата",
                code = "chat_message_id_missing",
            )
        val previousMessage = chatMessageDao.findTopByDialogIdAndIdLessThanOrderByIdDesc(
            dialogId = dialogId,
            messageId = markerId,
        )
        ...
        participantState.lastReadMessageId = previousMessage?.id
        participantState.lastReadAt = OffsetDateTime.now()
    }
}
